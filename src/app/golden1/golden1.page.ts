...
@Component({
  selector: 'app-golden1',
  templateUrl: './golden1.page.html',
  styleUrls: ['./golden1.page.scss'],
  standalone: true,
...
export class Golden1Page implements OnInit {

  teamName: string = 'Starlight';
  searchQuery: string = '';
  selectedPlayers: any[] = [];
  ...
  ngOnInit() {
    ...
    // Load saved players from localStorage for team 2
  ...
  // Save selected players to localStorage (separate key for team 2)
  private saveSelectedPlayers() {
    localStorage.setItem('selectedPlayersTeam2', JSON.stringify(this.selectedPlayers));
    ...
    localStorage.setItem('teamBPlayers', JSON.stringify(this.selectedPlayers));
    
    console.log(`Saved ${this.selectedPlayers.length} players for ${this.teamName}`);
    console.log('Players saved to keys:', `${teamName}Players`, 'teamBPlayers');
  }

  // Load selected players from localStorage (separate key for team 2)
  private loadSelectedPlayers() {
    const savedPlayers = localStorage.getItem('selectedPlayersTeam2');
    if (savedPlayers) {
  ...
  clearAllPlayers() {
    ...
    localStorage.removeItem('selectedPlayersTeam2');
  }
